...
class RefreshData extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($type == 'file') {
            ...
            $output->writeln('已从本地加载数据完毕，共'.$this->count.'条');
            $output->writeln('数据最后更新时间:'.$data['time']);
        } else {
            //从京东获取省市县数据
            $this->provinceList = [
                '1'     => '北京',
                ...
                foreach ($this->provinceList as $id => $province) {
                    ...
                    if ($cityList !== false) {
                        foreach ($cityList as $city) {
                            $output->writeln('获取数据成功:'.$province.$city['name']);
                            $_cityList[$city['id']] = [
                                'id'        => $city['id'],
                                'name'      => $city['name'],
                                'parent_id' => $id,
                            ...
                            if ($areaList !== false) {
                                foreach ($areaList as $area) {
                                    $output->writeln('获取数据成功:'.$province.$city['name'].$area['name']);
                                    $_areaList[$area['id']] = [
                                        'id'          => $area['id'],
                                        'name'        => $area['name'],
                                        'type'        => 'area',
                                    ...
                                    if ($streetList !== false) {
                                        foreach ($streetList as &$street) {
                                            ...
                                            $output->writeln('获取数据成功:'.$province.$city['name'].$area['name'].$street['name']);
...
//                        dump($area['street_list']);
    ...
    public function parseData($id,OutputInterface $output)
    {
        ...
        if ($data === false) {
            $output->writeln('获取数据失败.');
        } else {
            $data = $this->parseJson($data);
            if ($data === false) {
                $output->writeln($this->url);
                $output->writeln('解析数据失败.');
            } else {
                return $data;
            }
        }
